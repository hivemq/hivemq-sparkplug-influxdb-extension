...
. In the folder `hivemq-sparkplug-extension`, modify the `conf/config.properties` file to fit your needs.
...
The Sparkplug Monitoring extension uses its own configuration file `conf/config.properties`.
